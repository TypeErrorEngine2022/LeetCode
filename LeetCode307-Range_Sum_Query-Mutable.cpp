...
template <typename Item>
int IndexPriorityQueue<Item>::diffSum(int right)
{  
    ...
    while (!Q.empty()){
        ...
        if (2 * key <= m_size) Q.push(2 * key);
        if (2 * key + 1 <= m_size) Q.push(2 * key + 1);
        Q.pop();
    }
...
public:
    ...
    int sumRange(int left, int right) {
        int diffSum = tree.diffSum(right);
